...
8.6 Tabla access_separation_rule (v5.4.0)
-----------------------------------------
...
.. code-block:: sql
 ...
 -- Tabla principal de reglas SoD (v5.4.0 — sustituye a function_separation_rules v5.2.1)
 -- Los conjuntos de funciones conflictivas se modelan con M2M (no FK binario).
 CREATE TABLE access_separation_rule (
 id INT AUTO_INCREMENT PRIMARY KEY,
 ...
 CONSTRAINT uk_sod_code UNIQUE (code),
 CONSTRAINT chk_sod_state CHECK (state IN ('ENABLED', 'DISABLED'))
 ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;



8.7 Tablas M2M de conjuntos de funciones (v5.4.0)
--------------------------------------------------
...
.. code-block:: sql
 ...
 -- Tablas M2M auto-gestionadas por Django para functions_set_a y functions_set_b.
 -- Sustituyen a function_separation_rule_details (v5.2.1).
 CREATE TABLE access_separation_rule_functions_set_a (
 id INT AUTO_INCREMENT PRIMARY KEY,
 separationrule_id INT NOT NULL,
 function_id INT NOT NULL,
 CONSTRAINT fk_set_a_rule FOREIGN KEY (separationrule_id)
   REFERENCES access_separation_rule(id) ON DELETE CASCADE,
 CONSTRAINT fk_set_a_fn   FOREIGN KEY (function_id)
   REFERENCES access_function(id) ON DELETE CASCADE,
 CONSTRAINT uk_set_a UNIQUE (separationrule_id, function_id)
 ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
 
 CREATE TABLE access_separation_rule_functions_set_b (
 id INT AUTO_INCREMENT PRIMARY KEY,
 separationrule_id INT NOT NULL,
 function_id INT NOT NULL,
 CONSTRAINT fk_set_b_rule FOREIGN KEY (separationrule_id)
   REFERENCES access_separation_rule(id) ON DELETE CASCADE,
 CONSTRAINT fk_set_b_fn   FOREIGN KEY (function_id)
   REFERENCES access_function(id) ON DELETE CASCADE,
 CONSTRAINT uk_set_b UNIQUE (separationrule_id, function_id)
 ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
.. code-block:: sql
 ...
 -- Insertar reglas SoD (v5.4.0 — tabla access_separation_rule)
 INSERT INTO access_separation_rule (code, name, description, state) VALUES
 ('SOD-001', 'pipeline_audit_separation',
  'Pipeline operations cannot audit themselves', 'ENABLED'),
 ('SOD-002', 'user_audit_separation',
  'User management cannot audit themselves', 'ENABLED'),
 ('SOD-003', 'access_audit_separation',
  'Permission management cannot audit themselves', 'ENABLED');
 
 -- Los conjuntos M2M se cargan via management command (IACT-api):
 -- python manage.py create_separation_rules
 -- El command asigna functions_set_a y functions_set_b usando .set().
...
.. code-block:: python
 ...
 """
 Regla de Separacion de Deberes (SoD) — modelo v5.4.0.
 ...
 - functions_set_a: Funciones de pipeline (PIP-001 a PIP-004)
 - functions_set_b: Funciones de auditoria (AUD-001 a AUD-004)

 Hallazgo F0-H-003: el modelo v5.2.1 usaba FKs binarios (FunctionSeparationRule
 + FunctionSeparationRuleDetail) que solo permitian pares de funciones.
 El modelo v5.4.0 usa M2M para conjuntos multiples por lado.
 """
   ...
   max_length=20,
   unique=True,
   help_text="Identificador canonico (SOD-001, SOD-002, SOD-003)"
   )
   name = models.CharField(
   max_length=200,
   ...
   description = models.TextField(
   blank=True,
   help_text="Descripcion de la restriccion"
   )
...
.. code-block:: python
 ...
 # Obtener reglas SoD activas (v5.4.0 — state=ENABLED)
 active_rules = SeparationRule.objects.filter(
 state=SeparationRule.STATE_ENABLED
 )
 
 for rule in active_rules:
 # Obtener codigos de funciones de cada conjunto M2M
 codes_set_a = set(
 rule.functions_set_a.values_list('code', flat=True)
 )
 codes_set_b = set(
 rule.functions_set_b.values_list('code', flat=True)
 )
 
 # Validar conflicto
 if new_function_id in codes_set_a:
 if current_functions & codes_set_b:
 return False, (
 f"Violacion SoD: {rule.name}. "
 f"Regla: {rule.code}"
 )
 
 if new_function_id in codes_set_b:
 if current_functions & codes_set_a:
 return False, (
 f"Violacion SoD: {rule.name}. "
 f"Regla: {rule.code}"
 )
...
.. code-block:: sql
 ...
 -- Actualizar 3 reglas SoD (v5.2.0 → v5.2.1: nombres en ingles)
 -- NOTA v5.4.0: la tabla function_separation_rules fue reemplazada por
 -- access_separation_rule (modelo M2M). Estos UPDATE son parte del script
 -- de migracion v5.2.0 → v5.2.1 y se conservan como documentacion historica.
 -- La migracion v5.2.1 → v5.4.0 la gestionan las migraciones Django de IACT-api.
 ...
 -- NOTA v5.4.0: function_separation_rule_details fue eliminada. No aplica ALTER.
