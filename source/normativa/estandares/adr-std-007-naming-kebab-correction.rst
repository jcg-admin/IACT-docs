.. meta::
 :artefacto: ADR_STD_007_NAMING_KEBAB_CORRECTION
 :tipo: ADR
 :dominio: normativa
...
==============================================================
ADR STD-007: Naming Kebab Correction (corrige heterogeneidad)
==============================================================
 ...
 **Corrige (parcialmente)** a
 :doc:`adr-std-007-naming-heterogeneity-accepted`.

1. Contexto
===========

Este ADR corrige
:doc:`adr-std-007-naming-heterogeneity-accepted` emitido el
2026-04-29. El ADR original aceptó heterogeneidad de naming por
categoría (5 dialectos coexistiendo). El análisis posterior
identificó que **la heterogeneidad es daño estructural observable
hoy**, no deuda aceptable.

2. Distinción importante: Corrección ≠ Revocación
=================================================
 ...
 * - Aspecto
   - Revocación arbitraria
   - Corrección temprana
 * - Temporalidad
   - Cualquier momento
   - Ventana < 48h del original
 * - Justificación
   - "Cambié de opinión"
   - Premisas invalidadas con análisis
 * - Evidencia
   - Ninguna nueva
   - Análisis cuantitativo del corpus
 * - Patrón
   - Norm churn
   - Proceso de revisión funcionando

3. Premisas del ADR original que se invalidaron
===============================================

3.1 Premisa 1 (invalidada): "0 incidentes de heterogeneidad"
------------------------------------------------------------
...
La inconsistencia es **observable hoy**, en cualquier ``ls``. No
requiere monitoreo de 30-60 días.

3.2 Premisa 2 (invalidada): "Costo masivo, beneficio especulativo"
------------------------------------------------------------------

El análisis original calculó costo de migración linealmente. La
realidad es **no lineal por refs cruzadas**:
...
mucho mayor complejidad de refs. **El costo crece exponencial, no
linealmente.**

3.3 Premisa 3 (invalidada): "Forward-only mitiga"
-------------------------------------------------
 ...
 * - Directorio
   - ``kebab-case``
   - ``casos-uso/``, ``arquitectura-tecnica/``
...
- **Build verde 0/0/0** mantenido en cada batch.
  ...
  ``find source -type f -name '*.rst' ! -name 'index.rst' |
  awk -F/ '{print $NF}' | grep -E '[A-Z_]' | wc -l`` y comandos
  análogos. Una iteración previa de este ADR citó 276/38/48 —
  eran estimaciones parciales y quedan corregidas aquí.

Operación concentrada en 1-2 sesiones, con script idempotente +
PILOT en 1 archivo antes de bulk.

7. Salvaguarda anti norm-churn
==============================

**Commitment de estabilidad: 30 días sin nuevas modificaciones a
STD_007 después de v2.0.0.**

Esta cláusula se documenta aquí explícitamente. Si en 30 días
aparece evidencia que justifique nueva modificación, debe abrirse
WP propio con deep-review previo, igual que este.
...
- Refs externas (PRs históricos, marcadores en navegador) se
  ...
  commit del rename.
- ``git log --follow`` puede degradar levemente; mitigación:
  rename puro (sin tocar contenido) y commit consolidado.
 ...
 * - **Estándar referenciado**
   - :doc:`std-007-convencion-naming` (v2.0.0+)
