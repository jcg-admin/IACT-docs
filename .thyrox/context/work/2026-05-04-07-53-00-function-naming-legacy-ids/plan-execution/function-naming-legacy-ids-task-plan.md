...
- [ ] **T-001** `bounded-context-rbac.rst`: reemplazar `<<ACC-004>>`→
...
- [ ] **T-002** `bounded-context-audit.rst`: `<<AUD-001>>`→`<<view_audit_log>>`,
  `<<AUD-002>>`→`<<search_audit_log>>`, `<<AUD-003>>`→`<<export_audit_log>>`,
  `<<AUD-004>>`→`<<generate_compliance_report>>`.
- [ ] **T-003** `bounded-context-auth.rst`: `<<AUTH-004>>`→
  `<<view_all_active_sessions>>`, `<<AUTH-001>>`→`<<view_own_sessions>>`.
- [ ] **T-004** `bounded-context-alerts.rst`: ALR-NNN → codenames
  (view_alerts, configure_alerts, etc.).
- [ ] **T-005** `bounded-context-logs.rst`: LOG-NNN → codenames
  (view_application_logs, export_logs, etc.).

## Bloque B — Matriz de dependencias

- [ ] **T-006** `matriz-dependencias-uc-iact.rst`: reemplazar `ACC-NNN codename`
...
- [ ] **T-007** `grupos-funciones.rst`: reemplazar IDs en tablas de grupos.
- [ ] **T-008** `sod.rst`: reemplazar IDs en definición de reglas SoD.
- [ ] **T-009** `raci-rbac-iact/convenciones.rst`: reemplazar `SUP-001..003`
  con codenames.
- [ ] **T-010** `modulos/supervision/index.rst`: `SUP-NNN codename` → codename.

## Bloque D — Base cognitiva y modelo de dominio

- [ ] **T-011** `fnd-00-contexto-y-jerarquia.rst`: `AUD-003` → `export_audit_log`,
  `LOG-002` → `export_logs`.
- [ ] **T-012** `modelo-dominio-iact.rst`: `ALR-007` → `acknowledge_alert`.

## Bloque E — Normativos y procedimientos (verificar contexto)

- [ ] **T-013** `adr-gob-009-rbac-modelo-conceptual.rst`: verificar contexto
  y reemplazar IDs activos (no los que estén en contexto histórico).
- [ ] **T-014** `guia-completa-desarrollo-features.rst`: idem.
- [ ] **T-015** `proc-qa-002-estrategia-qa.rst`: idem.
- [ ] **T-016** `cnst-030-reglas-de-separacion-de-funciones-sod.rst`:
  verificar y reemplazar IDs de función (ACC-NNN en definición de SoD).
- [ ] **T-017** `tpl-br-decision-tipo.rst`: verificar templates.
- [ ] **T-018** `tpl-fr-documentacion-10-componentes.rst`: idem.
